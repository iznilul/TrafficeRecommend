...
import {getHotel} from '../../api/manage'
export default {
  components: {
    BaiduMap,
      BmlMarkerClusterer,
    ...
    methods: {
        ...
        getData(){
            getHotel({ScenicAreaProvince:this.province,ScenicAreaCity:this.city,ScenicAreaArea:this.area,Size:this.size}).then(res=>{
                // console.log(res.data)
                this.hotels=res.data
                // console.log(typeof Number(this.hotels[0].locationLng))
                this.handler(Number(this.hotels[0].locationLng),Number(this.hotels[0].locationLat))
                this.addPoints()
            })
        },
        addPoints () {
            ...
            for (var i = 0; i <this.hotels.length; i++) {
                const position = {lng: Number(this.hotels[i].locationLng),
                    lat: Number(this.hotels[i].locationLat),
                    province:this.hotels[i].province,
                    city:this.hotels[i].city,
                    area:this.hotels[i].area,
                    address:this.hotels[i].address,
                    name:this.hotels[i].name,
                    price:this.hotels[i].price,
                    recommend:this.hotels[i].recommends}
                s.push(position)
            }
            this.points=s
