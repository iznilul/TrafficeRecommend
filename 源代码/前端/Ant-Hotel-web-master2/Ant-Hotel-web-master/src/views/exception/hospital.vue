...
import {getHospital} from '../../api/manage'
export default {
  components: {
    BaiduMap,
      BmlMarkerClusterer,
    ...
    methods: {
        ...
        getData(){
            getHospital({ScenicAreaProvince:this.province,ScenicAreaCity:this.city,ScenicAreaArea:this.area,Size:this.size}).then(res=>{
                // console.log(res.data)
                this.hospitals=res.data
                // console.log(typeof Number(this.hospitals[0].locationLng))
                this.handler(Number(this.hospitals[0].locationLng),Number(this.hospitals[0].locationLat))
                this.addPoints()
            })
        },
        addPoints () {
            ...
            for (var i = 0; i <this.hospitals.length; i++) {
                const position = {lng: Number(this.hospitals[i].locationLng),
                    lat: Number(this.hospitals[i].locationLat),
                    province:this.hospitals[i].province,
                    city:this.hospitals[i].city,
                    area:this.hospitals[i].area,
                    address:this.hospitals[i].address,
                    name:this.hospitals[i].name,
                    recommend:this.hospitals[i].recommends}
                s.push(position)
            }
            this.points=s
