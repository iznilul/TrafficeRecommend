...
import random
import os
# if os.path.exists('entertainment.txt'):
#     os.remove('entertainment.txt')
cur.execute(DROP5)
cur.execute(CREATE5)
def getjson(loc):
	decodes=[]
			...
			if decodejson['results']:
				for eachone in decodejson["results"]:
					...
					try:
						...
						cur.execute(SAVEIN5, (str(
							i) , name , province , city , area , address , str(
							location_lat) , str(location_lng) , str(round(a, 1)) , str(round(b, 1))
											  ))
						conn.commit()
						with open('D:\python\s\BD\entertainment.txt', 'a') as f:  # 设置文件对象
