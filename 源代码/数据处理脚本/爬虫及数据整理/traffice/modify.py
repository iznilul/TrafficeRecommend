...
engine = create_engine('mysql+pymysql://root:123@localhost:3307/baidutraffice')
...
df3['departure_station_name'] = df1[df1['route_number'].shift(-1) == df1['route_number']].loc['departure_station_name'] = df1[
	'departure_station_name']+','+df1['arrive_station_name']
df3['arrive_station_name'] = df1[df1['route_number'].shift(-1) == df1['route_number']].loc['arrive_station_name'] = df1[
	'departure_station_name'].shift(-1)+','+df1['arrive_station_name'].shift(-1)
...
df2.to_sql(name='journeys_direct', con=engine, if_exists='replace', index=False)
df3.to_sql(name='journeys_indirect', con=engine, if_exists='replace', index=False)
