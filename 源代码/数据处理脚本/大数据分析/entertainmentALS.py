...
table='entertainmentUser'
DROP = "drop table if exists %s" % table
create = "create table %s (userId int,entertainmentId int,recommend float)" % table
insert = "insert into "+ table +" values(%s,%s,%s)"
spark = SparkSession.builder.master("local").appName("entertainmentALS").getOrCreate()

df=spark.read.text('file:///export/servers/pycharm_project/entertainment.txt')
attrs_rdd1=df.rdd.map(lambda x:x[0].split(',')).map(lambda x:Row(userId=1,entertainmentId=x[0],recommend1=x[8]))
attrs_rdd2=df.rdd.map(lambda x:x[0].split(',')).map(lambda x:Row(userId=2,entertainmentId=x[0],recommend2=x[9]))
