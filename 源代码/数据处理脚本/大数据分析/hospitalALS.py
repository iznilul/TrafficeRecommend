...
spark = SparkSession.builder.master("local").appName("hospitalALS").getOrCreate()

df=spark.read.text('file:///export/servers/pycharm_project/hospital.txt')
attrs_rdd1=df.rdd.map(lambda x:x[0].split(',')).map(lambda x:Row(userId=1,hospitalId=x[0],recommend1=x[8]))
attrs_rdd2=df.rdd.map(lambda x:x[0].split(',')).map(lambda x:Row(userId=2,hospitalId=x[0],recommend2=x[9]))
